...
const StudentForm = ({
  ...
  return (
    ...
    <CldUploadWidget uploadPreset='skooli' onSuccess={(result, { widget }) => {
      setImg(result.info);
      widget.close();
    }}
    >
    ...
    <div className="flex justify-between flex-wrap gap-4">
      ...
      <div className="flex flex-col gap-2 w-full md:w-1/4">
        ...
        <select className='ring-[1.5px] ring-gray-300 p-2 rounded-md text-sm w-full'
          {...register("classId")} defaultValue={data?.classId}>
            {classes.map(
              (classItem: {
                id: number;
                name: string;
        ...
        <button type='submit' className='bg-skooliBlue-400 text-white p-2 rounded-md'>
